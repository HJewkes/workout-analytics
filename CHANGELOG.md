...
- **`LoadSettings.chainsFullExtension` is a new REQUIRED field**, and `calculateFrameLoad`'s chain ramp is no longer hard-coded to a 0–1 range. The old ramp was the only place in the library that assumed the old position contract: `chainsFactor = clamp(1 − position)` collapses to 0 for any device-native position, and under metres never reaches 0 within a real rep. The ramp is now `clamp(1 − position / chainsFullExtension, 0, 1)`.
  ...
  - **The ramp's DIRECTION is a known open question, unchanged by this release.** The term is descending in extension, which is the opposite of physical barbell chains and — per `voltra-node-sdk` `src/sdk/voltra-client.ts:693-700`, documenting `setInverseChains` as "opposite of regular chains" — models the device's *inverse*-chains behaviour under the name `chains`. The formula is deliberately untouched here: flipping it moves every chains-set load and needs its own change with its own review. The tests pin current behaviour as a change-detector, explicitly not as a validated physical model. Tracked in `KNOWN-ISSUES-2026-07-27.md`.
