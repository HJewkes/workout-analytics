...
- [Position-scale boundary (v2)](#position-scale-boundary-v2)
...
| `repNumber` | `number` | |
| `rawSamplesJson` | `string` | **Opaque string** — serialized `{ concentric: WorkoutSample[], eccentric: WorkoutSample[] }`. Probed for JSON-parseability on read (corruption check, v5R-4); the parsed value is discarded. **The `position` values inside are on the scale of the writing version — see [Position-scale boundary](#position-scale-boundary-v2).** |
| `schemaVersion` | `number` | Overwritten on save. Load-bearing for the position-scale boundary: `1` = pre-2.0.0 scale, `>= 2` = metres. |
...
const MIGRATIONS: readonly Migration[];  // currently 2 entries: v1, v2
```
...
### Position-scale boundary (v2)

Source: `src/schema/migrations/002_position_metres.sql`.

WA 2.0.0 redefined `WorkoutSample.position` from a normalised 0–1 fraction to **cable extension in metres**. Sample streams are persisted verbatim inside `reps.raw_samples_json`, so a database written by 1.7.0 and read by 2.0.0 holds positions on **both** scales in the same table.

Migration v2 is a **marker, not a data migration.** It rewrites nothing:

```sql
PRAGMA user_version = 2;
```

Why no conversion: the old scale was device-dependent and was never recorded alongside the data, so there is no factor to convert by. A rewrite would have to infer one, corrupting every row it touched while reporting success. The producer side took the same decision — mark the boundary, never rewrite history to a scale you are inferring.

What the marker buys:

| Signal | Meaning |
| --- | --- |
| `reps.schema_version = 1` | `raw_samples_json` positions are on an **unspecified, pre-2.0.0** scale. |
| `reps.schema_version >= 2` | positions are **metres**. |
| `PRAGMA user_version` | mirrors the boundary at the SQLite level, readable without touching `__migrations`. |

This works because applying v2 raises `latestAppliedVersion` to `2`, and `prepareForSave` stamps that onto every row written from then on (see [`latestAppliedVersion` derivation](#latestappliedversion-derivation)). Rows already in the table keep `schema_version = 1`.

**Consumer obligations:**

- Do not compare absolute ROM, work or power across the boundary. A `getRepRangeOfMotion` over old rows is ~1000× smaller than over new ones, with no error raised.
- Ratio analytics are scale-invariant only *within* one scale, so a set whose reps span both versions cannot be analysed as one set — filter by `schema_version` or discard.
- Any `TechniqueBaseline.rom` / `expectedROM` distribution built from `schema_version = 1` rows is on the wrong scale and must be rebuilt.
